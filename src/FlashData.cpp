...
{
	bool homeSlotsEqual = true;
	for (size_t i = 0; i < 12; ++i)
	{
		if (homeSlotConfig[i] != other.homeSlotConfig[i])
		{
			homeSlotsEqual = false;
			break;
		}
	}
	return magic == other.magic
		&& homeSlotsMagic == other.homeSlotsMagic
...
void FlashData::SetDefaults()
{
	...
	for (uint8_t &slot : homeSlotConfig) { slot = 0; }
	magic = magicVal;
}
