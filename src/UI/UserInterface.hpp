...
namespace UI
{
	...
	extern void UpdateFanName(size_t fanIndex, const char data[]);
	extern void UpdateFanPercent(size_t fanIndex, int rpm);
	extern void UpdateGpOutName(size_t outputIndex, const char data[]);
	extern void UpdateGpOutCount(size_t count);
	extern void UpdateGpOutPercent(size_t outputIndex, int percent);
	...
	extern void SetMachineMode(const char* mode);
