...
// Lightweight decorative panel used by the 800x480 UI. It has no
// event handling and is intentionally cheap in RAM; all geometry is stored in
// the object and drawing happens directly into the framebuffer.
class ModernPanelField : public DisplayField
{
	PixelNumber height;
	Colour borderColour;
	uint8_t radius;
...
public:
	ModernPanelField(PixelNumber py, PixelNumber px, PixelNumber pw, PixelNumber ph,
			Colour fill, Colour border, uint8_t pradius = 12);
...
// Lightweight vector glyphs used by the modern 800x480 UI. They are rendered
// from lines/circles at runtime instead of using the legacy bitmap icon set.
...
class VectorIconButton : public SingleButton
{
	VectorIcon icon;
	const char * _ecv_array null text;
	LcdFont font;
...
public:
	VectorIconButton(PixelNumber py, PixelNumber px, PixelNumber pw, PixelNumber ph, VectorIcon pi,
				 event_t e, const char * _ecv_array null pt, LcdFont pf, int param = 0);
	VectorIconButton(PixelNumber py, PixelNumber px, PixelNumber pw, PixelNumber ph, VectorIcon pi,
				 event_t e, int textVal, LcdFont pf, int param = 0);
	VectorIconButton(PixelNumber py, PixelNumber px, PixelNumber pw, PixelNumber ph, VectorIcon pi,
				 event_t e, const char * _ecv_array null pt, LcdFont pf, const char * _ecv_array param);

	void Refresh(bool full, PixelNumber xOffset, PixelNumber yOffset) override;
	void SetIcon(VectorIcon newIcon) { if (icon != newIcon) { icon = newIcon; changed = true; } }
	void SetText(const char * _ecv_array null newText);
	void SetIntVal(int newVal) { if (val != newVal || !useIntValue) { val = newVal; useIntValue = true; changed = true; } }
	void SetPrintText(bool enabled) { if (printText != enabled) { printText = enabled; changed = true; } }
	void SetShowIcon(bool enabled) { if (showIcon != enabled) { showIcon = enabled; changed = true; } }
	void SetHeight(PixelNumber newHeight) { if (height != newHeight) { height = newHeight; changed = true; } }
};

// Interactive top-down XY work area used by the modern Move page. A touch is
// encoded in ButtonPress::index, allowing the UI event handler to map it to an
// absolute RRF X/Y move without changing the generic touch pipeline.
class BedMapField : public ButtonBase
{
	PixelNumber height;
...
protected:
	PixelNumber GetHeight() const override { return height; }
	void CheckEvent(PixelNumber tx, PixelNumber ty, int& bestError, ButtonPress& best) override;

public:
	BedMapField(PixelNumber py, PixelNumber px, PixelNumber pw, PixelNumber ph, event_t e, Colour grid, Colour dot);
	bool IsButton() const override { return true; }
	void Refresh(bool full, PixelNumber xOffset, PixelNumber yOffset) override;
	void SetBounds(float pminX, float pmaxX, float pminY, float pmaxY);
	void SetFrame(PixelNumber px, PixelNumber py, PixelNumber pw, PixelNumber ph, bool isCircular);
	void SetPosition(float px, float py);
	bool TouchToMachine(unsigned int packedTouch, float& px, float& py) const;
};

class ButtonWithText : public SingleButton
{
	static LcdFont font;
	PixelNumber customHeight = 0;
...
public:
	...
	void SetHeight(PixelNumber h) { if (customHeight != h) { customHeight = h; changed = true; } }
};
...
protected:
	Icon icon;
	PixelNumber customHeight = 0;
	PixelNumber GetHeight() const override { return customHeight != 0 ? customHeight : GetIconHeight(icon) + 2 * iconMargin + 2; }
...
public:
	...
	void SetLabel(const char* _ecv_array null pl) { if (label != pl) { label = pl; changed = true; } }
...
// Compact percent control for dense 800x480 I/O rows. It renders the short
// channel name at the left edge and the percentage right-aligned, avoiding
// clipping of values such as 100% in an 8-column layout.
class CompactPercentButton : public SingleButton
{
	LcdFont font;
	const char* _ecv_array prefix;
	VectorIcon icon;
	bool useIcon;
	int val;
...
public:
	CompactPercentButton(PixelNumber py, PixelNumber px, PixelNumber pw, const char* _ecv_array pp)
		: SingleButton(py, px, pw), font(DisplayField::defaultFont), prefix(pp), icon(VectorIcon::Fan), useIcon(false), val(0) {}
	CompactPercentButton(PixelNumber py, PixelNumber px, PixelNumber pw, VectorIcon pi)
		: SingleButton(py, px, pw), font(DisplayField::defaultFont), prefix(nullptr), icon(pi), useIcon(true), val(0) {}

	int GetValue() const { return val; }
	void SetValue(int pv) { if (val != pv) { val = pv; changed = true; } }
	void SetPrefix(const char* _ecv_array pp) { if (prefix != pp || useIcon) { prefix = pp; useIcon = false; changed = true; } }
	void SetIcon(VectorIcon pi) { if (!useIcon || icon != pi) { icon = pi; useIcon = true; changed = true; } }
	void Refresh(bool full, PixelNumber xOffset, PixelNumber yOffset) override;
};

// One compact field implements the whole Home-slot source picker, avoiding
// 17 separate button allocations on the memory-constrained PanelDue v3.
class HomeSlotPickerField : public ButtonBase
{
	PixelNumber height;
	uint8_t selected;
	uint8_t fanMask;
	uint8_t outputMask;
	uint8_t macroMask;
	const char* fanLabels[8];
	const char* outputLabels[8];
	const char* macroLabels[8];

	unsigned int CountBits(uint8_t mask) const;
	uint8_t ConfigForChoice(unsigned int choice) const;
...
public:
	HomeSlotPickerField(PixelNumber py, PixelNumber px, PixelNumber pw, PixelNumber ph, event_t e);
	bool IsButton() const override { return true; }
	int GetIParam(unsigned int index) const override;
	void SetSelected(uint8_t value) { if (selected != value) { selected = value; changed = true; } }
	void SetAvailability(uint8_t fans, uint8_t outputs, uint8_t macros = 0)
	{
		if (fanMask != fans || outputMask != outputs || macroMask != macros) { fanMask = fans; outputMask = outputs; macroMask = macros; changed = true; }
	}
	void SetFanLabel(size_t index, const char* label)
	{
		if (index < 8 && fanLabels[index] != label) { fanLabels[index] = label; changed = true; }
	}
	void SetOutputLabel(size_t index, const char* label)
	{
		if (index < 8 && outputLabels[index] != label) { outputLabels[index] = label; changed = true; }
	}
	void SetMacroLabel(size_t index, const char* label)
	{
		if (index < 8 && macroLabels[index] != label) { macroLabels[index] = label; changed = true; }
	}
	void Refresh(bool full, PixelNumber xOffset, PixelNumber yOffset) override;
};
