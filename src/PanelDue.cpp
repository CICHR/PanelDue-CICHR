...
static uint8_t gpOutNameRequestMask = 0;
static uint8_t gpOutLivePollCounter = 0;
static int8_t gpOutNameQueryInFlight = -1;
...
static FieldTableEntry fieldTable[] =
{
	...
	{ rcvStateGpOutPwm,				"state:gpOut^:pwm" },
	{ rcvStateMachineMode,			"state:machineMode" },
...
#endif
#if FETCH_STATE
	{ .event = rcvOMKeyState, .seqid = rcvSeqsState, .lastSeq = 0, .state = SeqStateInit, .key = "state", .flags = "vni" },
#endif
#if FETCH_VOLUMES
...
void RequestGpOutName(size_t outputIndex)
{
	if (outputIndex < 8)
	{
		gpOutNameRequestMask |= (uint8_t)(1u << outputIndex);
	}
}
...
static void ProcessReceivedValue(StringRef id, const char data[], const size_t indices[])
{
	...
	// State section
	case rcvStateGpOutPwm:
		{
			float f;
			if (GetFloat(data, f) && f >= 0.0f && f <= 1.0f)
			{
				UI::UpdateGpOutPercent(indices[0], (int)((f * 100.0f) + 0.5f));
			}
		}
		break;

	case rcvStateMachineMode:
		UI::SetMachineMode(data);
		break;
	...
	case rcvPushResponse:
		if (gpOutNameQueryInFlight >= 0)
		{
			const int8_t queriedOutput = gpOutNameQueryInFlight;
			gpOutNameQueryInFlight = -1;
			UI::UpdateGpOutName((size_t)queriedOutput, data);
			break;
		}
...
static void ProcessArrayEnd(const char id[], const size_t indices[])
{
	...
	else if (currentResponseType == rcvOMKeyState && strcasecmp(id, "state:gpOut^") == 0)
	{
		UI::UpdateGpOutCount(indices[0]);
	}
...
int main(void)
{
	...
	if (rstc_get_reset_cause(RSTC) != RSTC_SOFTWARE_RESET && _esplash[0] == DISPLAY_X && _esplash[1] == DISPLAY_Y)
	{
		...
		} while (SystemTick::GetTickCount() - now < SplashScreenHoldTime);
	}
					...
					)
			{
				...
				else
				{
					...
					else
					{
						...
						bool sent = false;
						if (OkToSend() && gpOutNameQueryInFlight < 0 && gpOutNameRequestMask != 0)
						{
							for (size_t i = 0; i < 8; ++i)
							{
								const uint8_t bit = (uint8_t)(1u << i);
								if ((gpOutNameRequestMask & bit) != 0)
								{
									gpOutNameRequestMask &= (uint8_t)~bit;
									gpOutNameQueryInFlight = (int8_t)i;
									SerialIo::Sendf("echo {exists(global.Out%dName) ? global.Out%dName : \"Out%d\"}\n", (int)i, (int)i, (int)i);
									sent = true;
									break;
								}
							}
						...
						if (!sent)
						{
							if (++gpOutLivePollCounter >= 5)
							{
								gpOutLivePollCounter = 0;
								SerialIo::Sendf("M409 K\"state\" F\"f\"\n");
							}
							else
							{
